...
router.post( '/value', function ( req, res ) {
    request
        ...
        .then( response => {
            ...
            } else if ( req.body.variable === "acmode" ) {
                res.status( 200 ).json( { cmd: 'value', subcmd: variable, data: { acmode: result }, success: true } );
            } else if ( req.body.variable === "doorstatus" ) {
                res.status( 200 ).json( { cmd: 'value', subcmd: variable, data: { doorstatus: result }, success: true } );
            } else if ( req.body.variable === "doorduration" ) {
                res.status( 200 ).json( { cmd: 'value', subcmd: variable, data: { doorduration: result }, success: true } );
            } else if ( req.body.variable === "power" ) {
                res.status( 200 ).json( { cmd: 'value', subcmd: variable, data: { power: result }, success: true } );
            } else if ( req.body.variable === "allvar" ) {
                console.log( result );
                res.status( 200 ).json( { cmd: 'value', subcmd: variable, data: JSON.parse( result ), success: true } );
            }
        } )
        .catch( err => {
            //console.log( err );
...
router.post( '/publish', function ( req, res ) {
    ...
    request
        ...
        .send( { args: JSON.stringify( req.body.data ) } )
        .then( response => {
            res.status( 200 ).json( { cmd: 'publish', success: true, status: req.body.data.publish } );
        } )
        .catch( err => {
...
var clockUnit = 3600; // 1 sec --> 60 secs * 60 mins = 1 hour 
