...
String myallvariables()
{
  return String::format("{\"t\":%d,\"light\":%s,\"led\":%s,\"led_sensor\":%d,\"door_sensor\":%d,\"Humidity\":%.2f,\"TemperatureC\":%.2f,\"TemperatureF\":%.2f,\"acmode\":%d,\"actemp\":%d,\"doorstatus\":%s,\"door_opentime\":%ld,\"power_consumption\":%.2lf}",
                        (int)Time.now(), smartLight.getStatusStr().c_str(), toggleLed.getStatusStr().c_str(),
                        smart_light_analogvalue, door_analogvalue, hum, cel, far, acmode, actemp, doorstatus ? "\"Open\"" : "\"Close\"",
                        door_opentime, power_consumption);
}
...
void setup()
{
  ...
  Particle.variable("allvar", myallvariables);
  // remote control
...
void loop()
{
  ...
  // Door status
  //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  if (door_analogvalue < 500)
  {
    doorstatus = false;
    door_opentime2 = 0; // reset open time if closed
  }
  else
  {
    doorstatus = true;
  ...
  if (doorstatus)
  {
    door_opentime = ((int)Time.now() - door_opentime2) ;
  }

  thermostat();
  // calculate the power used
  power_consumption = actime * 2750 / 3600 ; // 60s * 60 min = 1 hour, 750watt/hour
  statusStr = myallvariables();
  //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  // Serial Cmd Part
  //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  serialCmdProcessing();
